...
filter_threshold = sys.argv[1]; filter_threshold = int(filter_threshold);
...
def writing_into_file(heading, data, mode):
    ...
    file_name = ""
    if (mode == "w"):
        file_name = "CandidateItems.txt"
    else:
        file_name = "FreqItemsets.txt"
    """
...
user_businessRDD = sc.textFile(input_file_path).map(lambda e: e.split(",")).map(lambda e: (e[0], e[1]))
headers = user_businessRDD.take(1)
print("headers:",headers)
finalRDD = user_businessRDD.filter(lambda e: e[0] != headers[0][0])


finalRdd = finalRDD.map(lambda e: (e[0], e[1])).groupByKey().mapValues(lambda e: list(set(e))).filter(lambda e: len(e[1]) > filter_threshold).persist()  # SET removes duplicates. CHECK - can remove list()

total_no_of_baskets = finalRDD.count()


solution = SON_Algorithm(finalRdd, supportThreshold, total_no_of_baskets)
